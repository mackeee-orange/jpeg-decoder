...
/// Thumbnail dat is "(3n bytes) Packed (24-bit) RGB values for the thumbnail pixels, n = XThumbnail * YThumbnail"
#[derive(Clone, Copy, Debug, PartialEq)]
struct Thumbnail {
    pub width: u8,
    pub height: u8,
...
pub fn parse_jfif_app0<R: Read>(reader: &mut R, length: usize) -> Result<JfifApp0> {
    ...
    let units = match read_u8(reader)? {
    ...
    let x_density = read_u16_from_be(reader)?;
    let y_density = read_u16_from_be(reader)?;
